...
TEST(FFTTest, InverseFFTTest)
{
    ...
    for(int i = 0; i < length; i++)
	{
        ...
        //std::cout << "data[" << i << "] = { " << data_i_split.real[i] << ", " << data_i_split.imag[i] << " } " << std::endl;
	}
	...
	data_in_real = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, length*sizeof(float), data_i_split.real, &error_code);
	...
	data_in_imag = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, length*sizeof(float), data_i_split.imag, &error_code);
	...
	// clFFT_OUT_OF_PLACE
	data_out_real = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, length*sizeof(float), data_cl_split.real, &error_code);
	...
	data_out_imag = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, length*sizeof(float), data_cl_split.imag, &error_code);
    ...
    {
		error_code |= clFFT_ExecutePlannar(queue, plan, batchSize, dir, data_in_real, data_in_imag, data_out_real, data_out_imag, 0, NULL, NULL);
	}
	...
	error_code |= clEnqueueReadBuffer(queue, data_out_real, CL_TRUE, 0, length*sizeof(float), data_cl_split.real, 0, NULL, NULL);
	error_code |= clEnqueueReadBuffer(queue, data_out_imag, CL_TRUE, 0, length*sizeof(float), data_cl_split.imag, 0, NULL, NULL);
 ...
 //   for(int i = 0; i < length; i++)
	//{
 //       std::cout << "data[" << i << "] = { " << data_cl_split.real[i] << ", " << data_cl_split.imag[i] << " } " << std::endl;
	//}
	...
	for(int iter = 0; iter < numIter; iter++)
    {
		error_code |= clFFT_ExecutePlannar(queue, plan, batchSize, dir, data_out_real, data_out_imag, data_in_real, data_in_imag, 0, NULL, NULL);
	}
	...
	error_code |= clEnqueueReadBuffer(queue, data_in_real, CL_TRUE, 0, length*sizeof(float), data_cl_split.real, 0, NULL, NULL);
	error_code |= clEnqueueReadBuffer(queue, data_in_imag, CL_TRUE, 0, length*sizeof(float), data_cl_split.imag, 0, NULL, NULL);
 ...
 //   for(int i = 0; i < length; i++)
	//{
 //       std::cout << "data[" << i << "] = { " << data_cl_split.real[i] / length << ", " << data_cl_split.imag[i] / length << " } " << std::endl;
	//}

    for(int i = 0; i < length; i++)
	{
        EXPECT_NEAR(data_i_split.real[i], data_cl_split.real[i] / length, 0.000001);
        EXPECT_NEAR(data_i_split.imag[i], data_cl_split.imag[i] / length, 0.000001);
    }
