    ...
    double* resultState)
{
    ...
    //(*f)( (const double**)&state_temp_2, numNeurons, stateSize, 0, state_temp_1 );
...
int goloumb()
{
    ...
    std::vector<double> V_t, I_app_t, h_t, n_t, z_t, sAMPA_t, xNMDA_t, sNMDA_t, sGABAA_t;
    ...
    double* resultState )
{
    resultState[1] = ( ( ( states[indexOfNeuron][2] - states[indexOfNeuron][1] ) /
        states[indexOfNeuron][3] ) + ( states[indexOfNeuron][4] / states[indexOfNeuron][5] ) ) /
